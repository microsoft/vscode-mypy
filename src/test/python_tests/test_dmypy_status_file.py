...
def test_dmypy_args_generates_status_file_when_setting_is_none():
    """None daemonStatusFile values fall back to a generated status file."""
    _clear_dmypy_cache()
    old_root = lsp_server.DMYPY_STATUS_FILE_ROOT
    lsp_server.DMYPY_STATUS_FILE_ROOT = pathlib.Path(os.environ.get("TEMP", "/tmp"))
    try:
        settings = _make_settings("/workspace/none_setting", None)
        result = lsp_server._get_dmypy_args(settings, "run")
        assert "--status-file" in result
        idx = result.index("--status-file")
        assert result[idx + 1] != ""
