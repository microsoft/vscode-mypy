...
| mypy-type-checker.interpreter       | `[]`                                          | Path to a python interpreter to use to run the linter server. When set to `[]` interpreter for the workspace is obtained from `ms-python.python` extension. If set to some value, that value takes precedence, and Python extension is not queried for the interpreter.                                                                          |
